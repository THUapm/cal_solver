...
MathSolver 正是为弥补这一缺口而构建的智能体：它以学生手写解答为输入，自动识别步骤、对照标准解法进行逐步批改，并给出每一步的置信度与正误判断，辅以元验证机制防止批改本身的误判。系统还提供“举一反三”功能，帮助学生针对自己暴露的薄弱点进行针对性练习。通过将 LLM 推理与符号计算工具（SymPy、SciPy）结合，项目力图让“个性化步骤诊断”在工程上可靠落地。系统的目标是构建端到端的“拍照 → 识别 → 求解 → 批改 → 置信度报告”全流程自动化管线，无需中间人工干预。
...
在两道真实手写题上的端到端实测验证了系统可用性：
\begin{itemize}[nosep]
  \item Q-A1 齐次常微分方程：5 步批改全部正确判定，并自动生成 3 道相似题；
  \item Q-A2 Gronwall 引理证明：LLM 自动切分为 7 个原子步骤，前提链验证全部通过；
  \item 单用例端到端耗时 200-220 秒，主要瓶颈在 LLM 推理调用。
\end{itemize}
...
前端通过迭代消费核心编排层 \texttt{yield} 的事件流驱动实时进度，典型事件序列为：\texttt{started} → \texttt{step\_start} → \texttt{code\_done} / \texttt{mcp\_done} → \texttt{verifying} → \texttt{step\_graded}，每个事件对应 UI 上的一个状态更新，无需轮询。
...
\begin{enumerate}[nosep]
  \item 端到端手写 → 批改 Pipeline：从手写图片输入到步骤级批改报告的全自动流程，无需中间人工干预。
  \item 置信度校准评测体系：引入 ECE / Brier / Cohen's $\kappa$ 多指标量化 LLM 口述置信度的校准质量。
  \item 自研智能体编排架构：基于生成器模式的流式事件驱动架构，实现 USC + Verify-First + PARC 多范式组合。
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item 举一反三覆盖度：对于较难的证明题（如 Gronwall 引理），LLM 偶尔生成空集——找不到合适的相似题目，目前尚未解决。
\end{enumerate}

\subsection{未来工作}

\begin{enumerate}[nosep]
  \item 学科拓展——物理与电路：在现有微积分与概率论基础上，新增物理题（受力分析 / 运动学 / 能量守恒）与电路题（基尔霍夫定律 / 节点电压法 / 戴维南等效）两类模式。物理题需引入受力分解与能量守恒的步骤模板；电路题需支持节点 / 支路标注的图结构输入。
  \item 多模态推理能力：当前 OCR 依赖独立的 Vision 模型调用，未实现端到端的多模态推理。未来需新增图片理解模块（电路图 / 力学示意图识别）与物理情景理解能力（变量识别 + 受力分解），让 LLM 直接从示意图中提取物理量并组织推理链。
  \item 数据集收集与标注：按 PRM800K 风格构建步骤级标注集——采集高中 / 大学物理题图与电路题图，标注格式为“题干 + 标准解 + 每步 is\_correct + 预期置信度”，量级目标为 4-6 个主题 × 每个 4-5 题，用于验证批改准确性与置信度校准。
  \item 其他方向：引入检索增强生成（RAG）知识库以纳入教材与定理资源；实现更细粒度的前提链验证（不仅识别步骤间依赖，也评估前提被使用的正确性）；开发浏览器扩展或移动端接口，降低师生使用门槛。
\end{enumerate}
